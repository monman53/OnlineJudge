...
struct LCA {
    ...
    void dfs(const G<T> &g, int v, int pp, int dd, T cc) {
        p[0][v] = pp;
        d[v] = dd;
        c[v] = cc;
        for(auto e : g.e[v]){
            if(e.to == pp) continue;
            dfs(g, e.to, v, dd+1, cc+e.w);
        }
    }
...
int main() {
    ...
    for(int i=0;i<q;i++){
        int x, y;cin >> x >> y;x--;y--;
        int a1 = lca.query(x, k);
        int a2 = lca.query(k, y);
        LL c1 = lca.c[x] + lca.c[k] - 2LL*(lca.c[a1]);
        LL c2 = lca.c[k] + lca.c[y] - 2LL*(lca.c[a2]);
        printf("%lld\n", c1+c2);
    }
    return 0;
}
