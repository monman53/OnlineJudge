...
int main() {
    ...
    inc(i, 0, m){
        P a = conv[(i+m-1)%m]-conv[i];
        P b = conv[i]-conv[(i+m+1)%m];
        //cout << conv[i] << a << b << endl;
        a = P{-a.imag(), a.real()};
        b = P{-b.imag(), b.real()};

        double theta = acos(dot(a, b)/(abs(a)*abs(b)))/(2*M_PI);
        ans[i] = theta;
        //cout << theta << endl;
    }
    ...
    inc(i, 0, n){
        bool flag = true;
        inc(j, 0, m){
            if(EQ(pl[i].real(), conv[j].real()) && EQ(pl[i].imag(), conv[j].imag())){
                printf("%.8lf\n", ans[j]);
                flag = false;
                break;
            }
